...
import { Users, Plus, QrCode, Play, Sparkles, Printer, Download, X, CheckCircle2, Edit3, ArrowUp, ArrowDown, Save } from 'lucide-react';
import { QRCodeSVG } from 'qrcode.react';
import html2canvas from 'html2canvas';

export default function StudentManager({ students, onAddStudent, onUpdateStudent, onReorderStudents, onScanStudent }) {
  ...
  const handleSaveEdit = (e) => {
    ...
    const updated = {
      ...
      avatar: editAvatar,
      status: Number(editAttendance) >= 85 ? 'High Attendance Student' : 'Low Attendance Student',
      badgeColor: Number(editAttendance) >= 85 ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' : 'bg-rose-500/20 text-rose-300 border-rose-500/30',
    };
    ...
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="bg-[#0D1117] bg-theme-card border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-extrabold text-slate-100 uppercase tracking-tight font-mono">
              📋 STUDENT ROSTER • EDIT & REARRANGE PROFILES
            </h2>
          </div>
          <p className="text-xs text-slate-400 font-malayalam mt-1">
            Edit name, roll number, attendance %, avatar, and reorder class student cards.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#0D1117] border border-slate-800 rounded-3xl p-6 shadow-xl h-fit">
          ...
          <form onSubmit={handleSubmitNew} className="space-y-4 font-mono text-xs">
            <div>
        ...
        {/* Existing Student List with Reordering & Edit Buttons */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {students.map((s, idx) => {
              ...
              return (
                <div
                  key={s.id}
                  className="bg-[#0D1117] border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col justify-between hover:border-slate-700 transition-all group relative"
                >
                  <div>
                    {/* Top Student Header & Reorder Controls */}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-mono text-slate-400">
                            {s.rollNo} • {s.department || 'Computer Science'}
                          </p>
                        </div>
                      </div>

                      {/* Reorder Arrows & Attendance Badge */}
                      <div className="flex flex-col items-end gap-1">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => handleMoveStudent(idx, 'up')}
                            disabled={idx === 0}
                            className="p-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-amber-400 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                    ...
                    <div className="mt-3 flex items-center justify-between text-xs">
                      <span className={`px-2.5 py-0.5 rounded-full font-mono text-[10px] border ${s.badgeColor}`}>
                        {s.attendance >= 85 ? '🔒 Stay Home & Play Games' : '🟢 Enter Class'}
                      </span>

                      <button
                        onClick={() => handleOpenEditModal(s)}
                        className="px-2.5 py-1 rounded-lg bg-slate-800/80 border border-slate-700 hover:border-amber-500/40 text-amber-300 text-[11px] font-mono font-bold flex items-center gap-1 transition-all"
                      >
      ...
      {editingStudent && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in font-mono">
          <div className="bg-[#0D1117] border-2 border-amber-500/40 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative space-y-4">
            ...
            <div className="border-b border-slate-800 pb-3">
              <h3 className="text-base font-extrabold text-amber-400 uppercase tracking-wider flex items-center gap-2">
                ...
                <span>✏️ EDIT STUDENT PROFILE</span>
              </h3>
              <p className="text-xs text-slate-400 font-malayalam mt-0.5">
                Update name, attendance score, and roll details.
              </p>
            </div>
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4 text-xs">
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  <label className="text-slate-400">Attendance Score:</label>
                  <span className={`font-bold ${editAttendance >= 85 ? 'text-emerald-400' : editAttendance < 50 ? 'text-rose-400' : 'text-amber-400'}`}>
                    {editAttendance}%
                  </span>
                </div>
                <input
                ...
                <div className="flex justify-between text-[10px] text-slate-500 mt-1">
                  <span>0% (Low)</span>
                  <span>75% (Safe)</span>
                  <span>85%+ (Stay Home & Play Games)</span>
                </div>
              </div>

              <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-[11px]">
                <span className="text-slate-400">Door Outcome: </span>
                <strong className={editAttendance >= 85 ? 'text-rose-400' : 'text-emerald-400'}>
                  {editAttendance >= 85 ? '🔒 STAY HOME & PLAY GAMES' : '🟢 FORCE ENTER CLASSROOM'}
                </strong>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setEditingStudent(null)}
                  className="px-4 py-2 rounded-xl bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700 font-bold"
                >
                  CANCEL
                </button>
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-amber-500/20 border border-amber-400 text-amber-300 hover:bg-amber-500/30 font-extrabold flex items-center gap-1.5 shadow-md"
                >
              ...
              <div className="flex items-center justify-between bg-slate-900/80 p-3.5 rounded-xl border border-slate-800 text-left">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">{qrModalStudent.department || 'Computer Science'}</p>
                  </div>
                </div>
